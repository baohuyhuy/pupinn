...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const getToken = () => {
      return localStorage.getItem('hms_token') || localStorage.getItem('guest_token');
    };
    ...
    const socket = new WebSocket(`${wsUrl}/api/chat/ws?token=${encodeURIComponent(token)}`);
  ...
  }, [user, activeContact, refetchContacts]);
  ...
  const sendMessage = (imageUrl?: string) => {
    if ((!inputText.trim() && !imageUrl) || !activeContact || !ws || ws.readyState !== WebSocket.OPEN) {
      return;
    }
          ...
          <>
            ...
            <div className="h-16 border-b border-white/10 flex items-center px-6 sticky top-0 bg-slate-950/80 backdrop-blur-md z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-lg leading-none text-white">{activeContact.name}</h3>
                  <span className="text-xs text-slate-400 capitalize">{activeContact.role}</span>
                </div>
              </div>
            </div>
                  ...
                  return (
                    <div key={msg.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold mt-1
                        ${isMe ? 'bg-amber-500/20 text-amber-300' : 'bg-slate-800 text-slate-300'}
                      `}>
          ...
          {contacts.map(contact => (
            ...
            >
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-medium shadow-sm
                ${activeContact?.id === contact.id ? 'bg-amber-500/30' : 'bg-slate-800'}
              `}>
